...
const NixieTube: React.FC<NixieTubeProps> = ({ value, label, skin, font, colorMode, customColor, flickerEnabled }) => {
  ...
  // Generate a random delay for the flicker animation so tubes don't sync up
  const animationDelay = useMemo(() => `-${Math.random() * 5}s`, []);

  // 1. Hexagonal Anode Mesh Pattern
  ...
  // 3. Vertical Wire Texture (Cathode Connections)
  ...
  // 4. LED Acrylic Stack Texture (Vertical lines)
  ...
  const defaultShadowLED = `0 0 2px #d600ff, 0 0 8px #b300ff, 0 0 20px #8000ff`; // Purpleish default for LED
  ...
  if (colorMode === ClockColorMode.RAINBOW) {
    ...
    if (isCyber) {
        ...
        // For LED default, use the Rainbow animation class because the reference shows multi-colored digits often,
        // but if we want a static "default" it's purple. Let's stick to purple static for default.
    } else {
  ...
  const flickerClass = flickerEnabled ? 'voltage-flicker' : 'tube-glow';
  ...
  const activeStyle: React.CSSProperties = {
    ...
    opacity: 1,
    zIndex: 30,
    filter: isLED ? 'contrast(1.2)' : 'blur(0.4px) contrast(1.3)', // LED is sharper
    animationDelay: flickerEnabled ? animationDelay : '0s',
  };
  ...
  // --- SKIN STRUCTURE RENDER ---

  // 1. LED SKIN: Rectangular Acrylic Block
  if (isLED) {
     return (
        <div className="relative flex flex-col items-center mx-1 sm:mx-2 group">
           ...
           <div className="relative w-20 h-32 sm:w-24 sm:h-40 md:w-36 md:h-56 lg:w-44 lg:h-64 border-x-4 border-[#111] bg-black/40 backdrop-blur-sm shadow-xl rounded-sm overflow-hidden">
                ...
                {/* Acrylic Stack Effect Background */}
                ...
                <div className="absolute inset-0 top-4 flex items-center justify-center text-[5.5rem] sm:text-[7rem] md:text-[10rem] lg:text-[13rem] font-bold select-none z-20">
                    {digits.map((digit) => (
                        <span
                        key={digit}
                        className={`absolute transition-all duration-150 ${digit === value ? `${flickerClass} ${animationClass}` : ''}`}
                        style={digit === value ? activeStyle : inactiveStyle}
                        >
                        {digit}
                        </span>
                    ))}
           ...
           {/* Label */}
           {label && (
                <span className="mt-3 text-[10px] sm:text-xs font-mono font-bold tracking-[0.2em] uppercase opacity-40 text-gray-400">
                {label}
                </span>
            )}
        </div>
  ...
  // 2. STANDARD TUBE SKINS (Classic, Cyber, Light)
  
  let tubeContainerClasses = "";
  if (isClassic) {
    tubeContainerClasses = "bg-[#0a0505] border-white/10 shadow-[inset_0_0_30px_rgba(0,0,0,1),0_0_15px_rgba(20,0,0,0.5)]";
  } else if (isLight) {
    // BLUE GLASS STYLE
    tubeContainerClasses = "bg-[#02050a]/90 border border-[#2060ff]/50 shadow-[0_0_15px_rgba(0,100,255,0.4),inset_0_0_20px_rgba(0,40,100,0.6)] backdrop-blur-sm";
  } else {
    // Cyber
    tubeContainerClasses = "bg-[#000810]/60 border-cyan-400/20 shadow-[inset_0_0_20px_rgba(0,10,20,0.9),0_0_15px_rgba(0,255,255,0.1)] backdrop-blur-[1px]";
  }

  let baseClasses = "";
  if (isClassic) {
    baseClasses = "bg-gradient-to-b from-[#111] to-[#000] border-[#333]";
  } else if (isLight) {
    baseClasses = "bg-gradient-to-b from-[#1a1a1a] to-[#050505] border-t border-[#334455] shadow-[0_5px_10px_rgba(0,0,0,0.8)]";
  } else {
    baseClasses = "bg-gradient-to-b from-[#0a1520] to-[#050a10] border-cyan-800/30 backdrop-blur-md";
  }

  let innerGlowClass = "";
  if (isClassic) {
    innerGlowClass = "from-[#ff2200]/10 via-transparent to-black";
  } else if (isLight) {
     innerGlowClass = "from-[#0044ff]/10 via-transparent to-transparent";
  } else {
    innerGlowClass = "from-[#ff0033]/10 via-cyan-900/5 to-[#000510]";
  }

  return (
    <div className="relative flex flex-col items-center mx-1 sm:mx-2 group">
      {/* The Glass Nipple (Top Tip) */}
      <div className={`absolute -top-3 sm:-top-4 w-3 sm:w-4 h-4 rounded-full blur-[0.5px] border-l border-t z-10 transition-colors duration-500
        ${isCyber 
            ? 'bg-cyan-100/10 border-cyan-200/20 shadow-[0_0_10px_rgba(0,255,255,0.1)]' 
            : (isLight ? 'bg-[#2060ff]/20 border-[#4080ff]/40 shadow-[0_0_8px_rgba(0,100,255,0.4)]' : 'bg-white/20 border-white/40 shadow-[0_0_10px_rgba(255,255,255,0.2)]')
        }`}>
      </div>

      {/* The Glass Tube Container */}
      <div 
        className={`relative w-20 h-32 sm:w-24 sm:h-40 md:w-36 md:h-56 lg:w-44 lg:h-64 rounded-t-full rounded-b-xl overflow-hidden transition-all duration-500 ${tubeContainerClasses}`}
      >
        {/* Layer 0: Getter Flash */}
        ...
        {isLight && (
            <div className="absolute inset-0 border-[1px] border-white/5 rounded-t-full rounded-b-xl pointer-events-none z-20 shadow-[inset_0_0_10px_rgba(0,60,200,0.3)]"></div>
        )}
        ...
        {/* Layer 1: Internal Glow */}
        <div className={`absolute inset-0 bg-radial-gradient ${innerGlowClass} opacity-70`}></div>

        {/* Layer 2: Wire Structure */}
        <div 
          className="absolute inset-4 z-0 opacity-50 mix-blend-overlay pointer-events-none"
          style={{ backgroundImage: wireBg, backgroundSize: '20px 100%' }}
        ></div>

        {/* Layer 3: Hexagonal Anode Mesh */}
        <div 
          className="absolute inset-1 top-6 bottom-4 z-20 opacity-100 mix-blend-overlay pointer-events-none rounded-lg"
          style={{ backgroundImage: hexMeshBg, backgroundSize: '8px 14px' }}
        ></div>
        ...
        <div className={`absolute top-8 left-1.5 sm:left-3 w-1 sm:w-1.5 h-3/5 rounded-full blur-[0.8px] pointer-events-none z-40 
          ${isCyber 
            ? 'bg-gradient-to-b from-cyan-100/40 via-white/10 to-transparent' 
            : (isLight ? 'bg-gradient-to-b from-[#80b0ff]/60 via-[#4080ff]/20 to-transparent' : 'bg-gradient-to-b from-white/70 via-white/20 to-transparent')
          }`}>
        </div>
        
        <div className={`absolute top-10 right-1.5 sm:right-2 w-3 sm:w-4 h-1/2 rounded-full blur-sm pointer-events-none z-40
          ${isCyber 
            ? 'bg-gradient-to-b from-cyan-200/5 to-transparent' 
            : (isLight ? 'bg-gradient-to-b from-[#4080ff]/30 to-transparent' : 'bg-gradient-to-b from-white/10 to-transparent')
          }`}>
        </div>
        ...
        <div className={`absolute bottom-0 w-full h-6 pointer-events-none z-30 border-b-[6px] rounded-b-xl
          ${isCyber 
            ? 'bg-gradient-to-t from-[#001015]/90 to-transparent border-cyan-500/10' 
            : (isLight ? 'bg-gradient-to-t from-[#000a15]/90 to-transparent border-[#2060ff]/30' : 'bg-gradient-to-t from-black/80 to-transparent border-white/5')
           }`}>
        </div>
      </div>
      ...
      {/* Label */}
      {label && (
        <span className={`mt-3 text-[10px] sm:text-xs font-mono font-bold tracking-[0.2em] uppercase opacity-60 drop-shadow-sm transition-colors duration-500
          ${isCyber ? 'text-cyan-700' : (isLight ? 'text-[#406080]' : 'text-[#774444]')}`}>
          {label}
        </span>
      )}
    </div>
